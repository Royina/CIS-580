...
def reconstruct3D(transform_candidates, calibrated_1, calibrated_2):
  ...
  for candidate in transform_candidates:
    ...
    for i in range(calibrated_1.shape[0]):
      depths, residuals, _, _ = np.linalg.lstsq(np.hstack([calibrated_2[i].reshape((3,1)), -R@calibrated_1[i].T.reshape((3,1))]), T.reshape((3,1)), rcond=-1)
      lambdas[i] = depths.reshape((2,))
    lambdas = lambdas.T
    """ END YOUR CODE
    """
